...
Hoax:
* F1 Score: 0.5854
* Balanced Accuracy Score: 0.5321
* Area Under the ROC Curve: 0.5321

More Citations Metrics:
* F1 Score: 0.4987
* Balanced Accuracy Score: 0.4901
* Area Under the ROC Curve: 0.4901

Unreliable Sources Metrics:
* F1 Score: 0.3892
* Balanced Accuracy Score: 0.4969
* Area Under the ROC Curve: 0.4969
...
### CNN
The accuracy for the CNN model was around 51%, which is slightly better than the other two models we implemented. CNN is better for capturing semantic relationships in text classification, so having a data preprocessing method that compliments this (rather than one that observes word frequency) may aid in future performance increases. Employing methods like one-hot encoding or word2vec might lead to higher accuracy in future iterations. 
